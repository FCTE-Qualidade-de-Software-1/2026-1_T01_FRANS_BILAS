...
| **M4** | Taxa de Maturidade por Cobertura de Testes (TMCT) | 🛡️ Confiabilidade | Maturidade | `(Testes Passando / Total Executados) x 100` (+ % cobertura) |
| **M5** | Taxa de Disponibilidade Operacional (TDO) | 🛡️ Confiabilidade | Disponibilidade | `(Reqs HTTP 2xx-3xx / Total Reqs) x 100` |
| **M6** | Índice de Tolerância a Falhas (ITF) | 🛡️ Confiabilidade | Tolerância a Falhas | `(Falhas Tratadas / Total Testado) x 100` |
