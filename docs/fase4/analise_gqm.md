...
Os resultados são organizados por GOAL do GQM. Para cada métrica constam o resultado obtido e o nível de julgamento correspondente.

**Panorama geral:** das 9 métricas avaliadas, 4 foram classificadas como **Excelente**, 3 como **Bom** e 2 como **Insuficiente**. Os dois pontos críticos concentram-se na Confiabilidade (ausência de testes automatizados) e na Eficiência de Desempenho sob volume (degradação da exportação).

#### GOAL 1 — Adequação Funcional

| ID | Métrica | Resultado | Julgamento |
| :---: | :--- | :---: | :---: |
| **M1** | Índice de Completude Funcional (ICF) | 100,0% | Excelente |
| **M2** | Taxa de Correção Funcional (TCF) | 90,0% *(9/10)* | Bom |
| **M3** | Índice de Adequação à Tarefa (IAT) | 83,3% *(5/6)* | Bom |

#### GOAL 2 — Confiabilidade

| ID | Métrica | Resultado | Julgamento |
| :---: | :--- | :---: | :---: |
| **M4** | Taxa de Maturidade por Cobertura de Testes (TMCT) | 0 testes automatizados &sup1; | Insuficiente |
| **M5** | Taxa de Disponibilidade Operacional (TDO) | 100,0% *(200/200 req.)* | Excelente |
| **M6** | Índice de Tolerância a Falhas (ITF) | 80,0% *(12/15)* | Bom |
...
| **M8** | Utilização de Recursos sob Carga (URC) | RAM 32,6 MB &middot; 21 ms sob 50 usuários &sup2; | Excelente |
| **M9** | Degradação de Desempenho por Volume (DDVD) | 1,7 &rarr; 91 ms *(&times;53)* &sup3; | Insuficiente |

&sup1; A cobertura de 57% reportada pelas ferramentas é **nominal**: reflete o carregamento dos módulos no boot do Django, não testes de asserção reais.
&sup2; CPU do PostgreSQL praticamente ociosa (~0%) entre transações; pico de RAM de 32,6 MB sob 50 usuários simultâneos.
&sup3; O tempo de exportação CSV multiplica-se por cerca de 53 ao crescer de 100 para 10.000 itens (detalhamento em M9).
